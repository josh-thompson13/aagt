...
export default function QuickQuote({ className = '', variant = 'default', onApply }: QuickQuoteProps) {
  ...
  return (
    <Card className={`${className} ${isCompact ? 'p-4' : ''}`}>
      <CardHeader className={isCompact ? 'pb-4' : ''}>
        <h3 className={`flex items-center gap-2 ${isCompact ? 'text-lg' : 'text-2xl'} font-bold text-gray-900`}>
          ...
          <div className={`bg-gray-50 rounded-lg p-${isCompact ? '4' : '6'} space-y-3`}>
            ...
            <div className="border-t pt-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-2xl font-bold text-gray-900">{formatCurrency(totalRepayment)}</span>
              </div>
            </div>
